%MIXANDMIX_EXP07 script performs the experiment included in Figs. 4a and 4b
...
for n=1:NB
    ...
    %SPECTRODE    
    if Beta<1;tolSpe=1e-4;else tolSpe=1e-3;end
...
labelFig('$\gamma$','$\overline{\Delta f}$','\textbf{a) Averaged accuracy ($\mathbf{\delta\delta}$ LS)}',FontSizeA);
...
xt=set(gca,'xtick',BetaV(1:ju:end),'xticklabel',arrayfun(@(x) num2str(x,'%.2f'),BetaVPrev(1:ju:end),'un',0));
ax=gca;ax.FontSize=FontSizeC;
labelFig('$\gamma$','$t$ (s)','\textbf{b) Computation times ($\mathbf{\delta\delta}$ LS)}',FontSizeA);
hh=legend([h{1} h{2} h{3}],meth,'FontSize',FontSizeB,'Location','NorthWest','Interpreter','latex');
pos=get(hh,'Position');
pos(2)=pos(2)+0.045;
set(hh,'Position',pos);
